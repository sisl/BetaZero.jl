"""
MCTS solver with double progressive widening and pUCT.
...
Fields:
    ...
    next_action::Any
        ...
        The next action is determined based on the MDP, the state, `s`, and the current `PUCTStateNode`, `snode`.
...
"""
# TODO: @with_kw
mutable struct PUCTSolver <: AbstractMCTSSolver
    depth::Int
    exploration_constant::Float64
    n_iterations::Int
    max_time::Float64
...
mutable struct PUCTTree{S,A}
    ...
    function PUCTTree{S,A}(sz::Int=1000) where {S,A} 
        sz = min(sz, 100_000)
...
function insert_state_node!(tree::PUCTTree{S,A}, s::S, maintain_s_lookup=true) where {S,A}
    push!(tree.total_n, 0)
...
function insert_action_node!(tree::PUCTTree{S,A}, snode::Int, a::A, n0::Int, q0::Float64, maintain_a_lookup=true) where {S,A}
    push!(tree.n, n0)
    push!(tree.q, q0)
...
Base.isempty(tree::PUCTTree) = isempty(tree.n) && isempty(tree.q)

struct PUCTStateNode{S,A} <: AbstractStateNode
    tree::PUCTTree{S,A}
    index::Int
end

children(n::PUCTStateNode) = n.tree.children[n.index]
n_children(n::PUCTStateNode) = length(children(n))
isroot(n::PUCTStateNode) = n.index == 1


mutable struct PUCTPlanner{P<:Union{MDP,POMDP}, S, A, VE, PE, NA, RCB, RNG} <: AbstractMCTSPlanner{P}
    solver::PUCTSolver
    mdp::P
    tree::Union{Nothing, PUCTTree{S,A}}
...
Random.seed!(p::PUCTPlanner, seed) = Random.seed!(p.rng, seed)
