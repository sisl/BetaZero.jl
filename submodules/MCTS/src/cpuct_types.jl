"""
MCTS solver with double progressive widening and CPUCT (constrained PUCT).
...
Fields:
    ...
    next_action::Any
        ...
        The next action is determined based on the MDP, the state, `s`, and the current `CPUCTStateNode`, `snode`.
...
"""
# TODO: @with_kw
mutable struct CPUCTSolver <: AbstractMCTSSolver
    depth::Int
    exploration_constant::Float64
    n_iterations::Int
    max_time::Float64
...
mutable struct CPUCTTree{S,A}
    ...
    function CPUCTTree{S,A}(sz::Int=1000) where {S,A} 
        sz = min(sz, 100_000)
...
function insert_state_node!(tree::CPUCTTree{S,A}, s::S, maintain_s_lookup=true) where {S,A}
    push!(tree.total_n, 0)
...
function insert_action_node!(tree::CPUCTTree{S,A}, snode::Int, a::A, n0::Int, q0::Float64, f0::Float64, maintain_a_lookup=true) where {S,A}
    push!(tree.n, n0)
    push!(tree.q, q0)
    push!(tree.f, f0)
...
Base.isempty(tree::CPUCTTree) = isempty(tree.n) && isempty(tree.q)

struct CPUCTStateNode{S,A} <: AbstractStateNode
    tree::CPUCTTree{S,A}
    index::Int
end

children(n::CPUCTStateNode) = n.tree.children[n.index]
n_children(n::CPUCTStateNode) = length(children(n))
isroot(n::CPUCTStateNode) = n.index == 1


mutable struct CPUCTPlanner{P<:Union{MDP,POMDP}, S, A, VE, PE, FE, NA, RCB, RNG} <: AbstractMCTSPlanner{P}
    solver::CPUCTSolver
    mdp::P
    tree::Union{Nothing, CPUCTTree{S,A}}
    value_estimate::VE
    policy_estimate::PE
    failure_estimate::FE
    next_action::NA
...
function CPUCTPlanner(solver::CPUCTSolver, mdp::P) where P<:Union{POMDP,MDP}
    ve = convert_estimator(solver.estimate_value, solver, mdp)
    pe = convert_estimator(solver.estimate_policy, solver, mdp)
    fe = convert_estimator(solver.estimate_failure, solver, mdp)
    return CPUCTPlanner{P,
                      statetype(P),
                      actiontype(P),
                      typeof(ve),
                      typeof(pe),
...
Random.seed!(p::CPUCTPlanner, seed) = Random.seed!(p.rng, seed)


function CPUCTSolver(solver::PUCTSolver)
    return CPUCTSolver(;
        depth=solver.depth,
        exploration_constant=solver.exploration_constant,
        n_iterations=solver.n_iterations,
        max_time=solver.max_time,
        k_action=solver.k_action,
        alpha_action=solver.alpha_action,
        k_state=solver.k_state,
        alpha_state=solver.alpha_state,
        keep_tree=solver.keep_tree,
        enable_action_pw=solver.enable_action_pw,
        enable_state_pw=solver.enable_state_pw,
        check_repeat_state=solver.check_repeat_state,
        check_repeat_action=solver.check_repeat_action,
        tree_in_info=solver.tree_in_info,
        counts_in_info=solver.counts_in_info,
        rng=solver.rng,
        estimate_value=solver.estimate_value,
        estimate_policy=solver.estimate_policy,
        init_Q=solver.init_Q,
        init_N=solver.init_N,
        next_action=solver.next_action,
        default_action=solver.default_action,
        reset_callback=solver.reset_callback,
        show_progress=solver.show_progress,
        timer=solver.timer,
        final_criterion=solver.final_criterion,
    )
end
