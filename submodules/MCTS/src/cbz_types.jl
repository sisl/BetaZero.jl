...
Fields:
    ...
    next_action::Any
        ...
        The next action is determined based on the MDP, the state, `s`, and the current `CBZStateNode`, `snode`.
...
"""
# TODO: @with_kw
mutable struct CBZSolver <: AbstractMCTSSolver
    depth::Int
    exploration_constant::Float64
    n_iterations::Int
    max_time::Float64
...
function insert_action_node!(tree::CBZTree{S,A}, snode::Int, a::A, n0::Int, q0::Real, f0::Real, maintain_a_lookup=true) where {S,A}
    ...
    push!(tree.transitions, Vector{Tuple{Int,Float64,Float64}}[])
...
struct CBZStateNode{S,A} <: AbstractStateNode
    tree::CBZTree{S,A}
    index::Int
end

children(n::CBZStateNode) = n.tree.children[n.index]
n_children(n::CBZStateNode) = length(children(n))
isroot(n::CBZStateNode) = n.index == 1


mutable struct CBZPlanner{P<:Union{MDP,POMDP}, S, A, VE, PE, FE, NA, RCB, RNG} <: AbstractMCTSPlanner{P}
    solver::CBZSolver
    mdp::P
    tree::Union{Nothing, CBZTree{S,A}}
    value_estimate::VE
    policy_estimate::PE
    failure_estimate::FE
    next_action::NA
...
function CBZPlanner(solver::CBZSolver, mdp::P) where P<:Union{POMDP,MDP}
...
Random.seed!(p::CBZPlanner, seed) = Random.seed!(p.rng, seed)
