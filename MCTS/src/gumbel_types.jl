...
Fields:
    ...
    next_action::Any
        ...
        The next action is determined based on the MDP, the state, `s`, and the current `GumbelStateNode`, `snode`.
...
"""
# TODO: @with_kw
mutable struct GumbelSolver <: AbstractMCTSSolver
    depth::Int
    exploration_constant::Float64
    n_iterations::Int
    max_time::Float64
...
mutable struct GumbelTree{S,A}
    ...
    function GumbelTree{S,A}(sz::Int=1000) where {S,A} 
        sz = min(sz, 100_000)
...
function insert_state_node!(tree::GumbelTree{S,A}, s::S, maintain_s_lookup=true) where {S,A}
    push!(tree.total_n, 0)
...
function insert_action_node!(tree::GumbelTree{S,A}, snode::Int, a::A, n0::Int, q0::Float64, maintain_a_lookup=true) where {S,A}
    push!(tree.n, n0)
    push!(tree.q, q0)
...
Base.isempty(tree::GumbelTree) = isempty(tree.n) && isempty(tree.q)

struct GumbelStateNode{S,A} <: AbstractStateNode
    tree::GumbelTree{S,A}
    index::Int
end

children(n::GumbelStateNode) = n.tree.children[n.index]
n_children(n::GumbelStateNode) = length(children(n))
isroot(n::GumbelStateNode) = n.index == 1


mutable struct GumbelPlanner{P<:Union{MDP,POMDP}, S, A, VE, PE, NA, RCB, RNG} <: AbstractMCTSPlanner{P}
    solver::GumbelSolver
    mdp::P
    tree::Union{Nothing, GumbelTree{S,A}}
...
Random.seed!(p::GumbelPlanner, seed) = Random.seed!(p.rng, seed)
